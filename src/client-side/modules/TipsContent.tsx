...
import { copyCommandResultStyle, textTipsFieldStyle } from '../style/textFieldStyle.ts';
...
export function TipsContent({
    ...
    const copy = () => {
        if (textCopied) {
            return (
                <span style={{ ...copyCommandResultStyle(theme), alignSelf: 'flex-start' }}>
                    {textCopied && 'Copy !'}
                </span>
            );
        }
        if (failCopied) {
            return (
                <span style={{ ...copyCommandResultStyle(theme, true), alignSelf: 'flex-start' }}>
                    {failCopied && 'Failed to copy !'}
                </span>
            );
        }
        return (
            <IconButton
                onClick={() => handleCopy!(tipsDetails.command)}
                sx={{
    ...
    return (
        <>
            ...
            <Typography sx={descriptionStyle(fullContent, nbDescriptionLines)}>{tipsDetails.description}</Typography>
        </>
    );
}
